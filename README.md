# Orchestrate '26: Enterprise Multi-Agent AI Triage System

**Production-Ready LangGraph Triage Architecture for Tier-1 Support Automation**
...
This repository is designed for reproducibility. Follow these steps to initialize and run the pipeline.
...
API keys for Groq are required for both classification and synthesis. Create a `.env` file in the `code/` directory:
...
Ingest markdown documentation into the localized search indexes:
...
To process the support_tickets.csv file:

```bash
python batch.py
```
Results will be output to `support_tickets/output.csv`.

### 5. Interactive UI (Optional)

To test the agent interactively via terminal:
...
*Developed for the HackerRank Orchestrate '26 Competition. Focused on Safety, Grounding, and Professional Reliability.*
