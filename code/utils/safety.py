...
ESCALATION_TRIGGERS: dict[str, tuple[int, str, str]] = {
    r"(\bfraud\b|\bunauthorized\b.*\b(charge|transaction)\b|stolen card|card compromised|someone used my card)":
        (10, "Visa Fraud & Security Team", "Potential fraud or unauthorized transaction detected"),
    r"(dispute|chargeback|transaction dispute)":
        (5, "Visa Disputes Resolution Team", "Transaction dispute requires human review"),
    ...
    r"(proctoring.*wrong|false.*positive|incorrectly.*flagged|cheating.*flag|assessment.*disqualif|unfairly.*penaliz)":
        (5, "HackerRank Integrity & Appeals Team", "Proctoring false positive — affects candidate career"),
...
def detect_pii(text: str) -> bool:
    ### use of this function: detect pii
...
def redact_pii(text: str) -> str:
    ### use of this function: redact pii
...
def get_risk_score(text: str) -> tuple[int, Optional[str], Optional[str]]:
    ### use of this function: get risk score
    """
